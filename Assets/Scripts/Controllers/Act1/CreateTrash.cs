...
public class CreateTrash : MonoBehaviour
{
    ...
    private void Update()
    {
        if (_sceneTimer.GetElapsedTime() < _acte1Data.acte1Timer) 
        {
            if (_lastSpawnTime + _trashUpdatedCD < _sceneTimer.GetElapsedTime())
            {
                Debug.Log("CreatedTrash");
                CreatingTrash();
            }
            _trashUpdatedCD = Mathf.Lerp(_acte1Data.trashSpawnCD, _acte1Data.trashScaledCD, _sceneTimer.GetElapsedTime() / _acte1Data.acte1Timer);
        }
    }
